...
const ALLOWED_INVOKE_CHANNELS = new Set([
  'llm:ping', 'llm:chat', 'llm:stream', 'model:set', 'llm:listModels', 'llm:health', 'llm:cancelStream',
  'state:get', 'state:set', 'state:update', 'state:export', 'state:import',
  'settings:get', 'settings:update',
  'session:list', 'session:save', 'session:load', 'session:search', 'session:delete', 'session:rename', 'session:metadata', 'session:repairIndex', 'session:export', 'session:import-bundle',
  'command:list', 'command:run',
  'llm:bridge:get', 'llm:bridge:test', 'llm:bridge:save',
  'rgb:status', 'rgb:applyMood',
  'audit:append', 'audit:tail', 'audit:verify',
  'system:stats',
...
contextBridge.exposeInMainWorld('api', {
  ...
  session: {
     ...
     */
    save: (name, data, passphrase) => ipcRenderer.invoke('session:save', name, data, passphrase),
    /**
     * Load and decrypt a session.
     * @param {string} name
     * @param {string} passphrase
     */
    load: (name, passphrase) => ipcRenderer.invoke('session:load', name, passphrase)
    ,
    /**
    ...
    export: (name, peerId) => ipcRenderer.invoke('session:export', name, peerId),
    /** Import and verify a NeuralLink™ bundle from a local path. */
    importBundle: (filePath) => ipcRenderer.invoke('session:import-bundle', filePath)
  },
  command: {
  ...
  logger: {
     ...
     */
    log: (level, message, meta) => ipcRenderer.invoke('log:log', level, message, meta),
  ...
  identity: {
    ...
    trustPeer: (deviceId, pubKeyPem, label) => ipcRenderer.invoke('identity:trust-peer', deviceId, pubKeyPem, label),
    /** Remove a peer device from the trust store. */
    revokePeer: (deviceId) => ipcRenderer.invoke('identity:revoke-peer', deviceId),
  ...
  daemon: {
    ...
    onTransferProgress: (fn) => ipcRenderer.on('transfer-progress', (_e, data) => fn(data))
  }
});
...
ipcRenderer.on('llm-stream-error', (_event, message) => {
  window.dispatchEvent(new CustomEvent('llm-stream-error', { detail: message }));
});
