...
function atomicRead(targetPath) {
  ...
  if (!isAllowed) throw new Error("CAPABILITY_DENIED: FS_OUT_OF_BOUNDS");
  ...
  try {
    ...
    if (e.code === 'ELOOP') throw new Error("FS_BOUNDARY_VIOLATION: SYMLINK_REJECTED");
    throw e;
  }
  ...
  try {
    ...
    if (!stats.isFile()) throw new Error("CAPABILITY_DENIED: NOT_A_FILE");
    
    // Ensure file hasn't been swapped for a link (Double verification for non-O_NOFOLLOW platforms)
    if (stats.isSymbolicLink()) throw new Error("FS_BOUNDARY_VIOLATION: SYMLINK_DETECTED");

    const buffer = Buffer.alloc(stats.size);
    const bytesRead = fs.readSync(fd, buffer, 0, stats.size, 0);
    
    if (bytesRead !== stats.size) throw new Error("CAPABILITY_DENIED: PARTIAL_READ");
