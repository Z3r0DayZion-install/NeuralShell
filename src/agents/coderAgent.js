...
export class CoderAgent extends BaseAgent {
  ...
  async executeTask(task) {
    if (task.type === 'generate_code') {
      return this.generateAndRun(task.data.spec);
    }
    if (task.type === 'invent_tool') {
      return this.inventTool(task.data.name, task.data.spec);
    }
  ...
  async generateAndRun(spec) {
    ...
    if (spec.includes('factorial')) {
      code = 'function f(n){return n<=1?1:n*f(n-1)}; console.log(f(5))';
    } else if (spec.includes('date')) {
