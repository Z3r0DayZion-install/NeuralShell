...
import networkBroker from '../kernel/network.js';

async function omegaFetch(url, options = {}) {
  const res = await networkBroker.safeFetch({
    url,
  ...
  const text = Buffer.from(res.data, 'base64').toString('utf8');
...
class OAuth2Provider {
  ...
  async exchangeCode(code, codeVerifier = null) {
    ...
    const response = await omegaFetch(this.tokenEndpoint, {
      method: 'POST',
      headers: {
  ...
  async revokeToken(token, tokenType = 'access_token') {
    ...
    try {
      const response = await omegaFetch(this.tokenEndpoint.replace('/token', '/revoke'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
...
class OIDCTokenVerifier {
  ...
  async fetchJWKS() {
    ...
    try {
      const response = await omegaFetch(this.jwksUri);
