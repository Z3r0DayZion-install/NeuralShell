...
import { EventStoreClient, getEventStore } from './eventStore.js';
import { 
  DecisionEvent, 
...
/**
 * Configuration for Decision Intelligence Engine
 */
export interface DecisionEngineConfig {
  /** Kafka brokers for event storage */
  brokers?: string[];
  
  /** Kafka topic for decision events */
  topic?: string;
  
  /** Client ID for Kafka */
  clientId?: string;
  
  /** Enable automatic connection on first use */
  autoConnect?: boolean;
...
export class DecisionIntelligenceEngine {
   ...
   * Requirements: 1.1, 1.2, 15.2
   */
  async recordDecision(event: PartialDecisionEvent): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to record decision: ${(error as Error).message}`);
    }
  }
   ...
   */
  async recordDecisionBatch(events: PartialDecisionEvent[]): Promise<void> {
    if (!this.connected && this.config.autoConnect) {
      await this.connect();
    }
    ...
    try {
      ...
      throw new Error(`Failed to record decision batch: ${(error as Error).message}`);
    }
  }
...
// Singleton instance
let decisionEngineInstance: DecisionIntelligenceEngine | null = null;
 ...
 */
export function getDecisionEngine(config?: DecisionEngineConfig): DecisionIntelligenceEngine {
