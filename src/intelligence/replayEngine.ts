...
export interface ReplayConfig {
  ...
  /** Enable sandbox mode (isolated from production) */
  sandbox: boolean;
  
  /** Replay speed multiplier (1x to 100x) */
  speed?: number;
  ...
  onEvent?: (event: DecisionEvent, replayTime: number) => void | Promise<void>;
...
export interface ReplayResult {
  ...
  /** Total real time taken in milliseconds */
...
/**
 * Difference types for outcome comparison
 */
...
/**
 * A single difference between original and replayed outcomes
 */
 ...
 */
export interface OutcomeComparisonReport {
  /** Time range compared */
  timeRange: TimeRange;
  ...
  /** Number of events with differences */
  differingEvents: number;
  
  /** Percentage of matching events */
  matchPercentage: number;
  
  /** All event comparisons */
  comparisons: EventComparison[];
  
  /** Summary of differences by type */
  differenceSummary: Record<DifferenceType, number>;
...
/**
 * Configuration for outcome comparison
 */
export interface OutcomeComparisonConfig {
  /** Time range to compare */
  timeRange: TimeRange;
  
  /** Original events to compare against */
  originalEvents?: DecisionEvent[];
  
  /** Replayed events to compare */
  replayedEvents?: DecisionEvent[];
  ...
  /** Maximum number of top differences to include in report */
  maxTopDifferences?: number;
}

/**
 * Reconstructed system state at a point in time
 */
export interface ReconstructedState {
  /** Timestamp of the state */
  timestamp: number;
  
  /** System state by component */
  ...
  /** Whether this state was loaded from cache */
  fromCache: boolean;
}

/**
 * State for a specific system component
 */
export interface ComponentState {
  ...
  /** Last decision made by this component */
  lastDecision?: DecisionEvent;
 ...
 */
export interface StateReconstructionConfig {
  /** Target timestamp to reconstruct state for */
  targetTimestamp: Date | number;
  
  /** Optional start time (defaults to beginning of time) */
  startTime?: Date | number;
...
export class ReplayEngine {
  private queryAPI: DecisionQueryAPI;
  private isReplaying: boolean = false;
  private shouldStop: boolean = false;
  private stateSnapshotCache: Map<number, StateSnapshot> = new Map();
  private readonly DEFAULT_SNAPSHOT_INTERVAL = 60000; // 1 minute in milliseconds
   ...
   * Requirements: 3.4
   */
  async compareOutcomes(config: OutcomeComparisonConfig): Promise<OutcomeComparisonReport> {
    ...
    try {
      ...
      const originalMap = new Map<string, DecisionEvent>();
      for (const event of originalEvents) {
        originalMap.set(event.event_id, event);
      }

      const replayedMap = new Map<string, DecisionEvent>();
      ...
      const differenceSummary: Record<DifferenceType, number> = {
        action_parameters: 0,
        outcome_status: 0,
      ...
      const allDifferences: OutcomeDifference[] = [];
      ...
      const report: OutcomeComparisonReport = {
        timeRange: config.timeRange,
        totalEvents,
        matchingEvents,
        differingEvents,
      ...
      throw new Error(`Outcome comparison failed: ${(error as Error).message}`);
    }
  }
  ...
  private compareEvent(
    originalEvent: DecisionEvent,
    replayedEvent: DecisionEvent | undefined,
    numericTolerance: number
  ): EventComparison {
    const differences: OutcomeDifference[] = [];
   ...
   * @private
   */
  private compareActionParameters(
    original: ActionTaken,
    replayed: ActionTaken,
    differences: OutcomeDifference[],
    tolerance: number
  ): void {
    // Compare action type
   ...
   * @private
   */
  private compareImpactMeasurements(
   ...
   * @private
   */
  private valuesMatch(value1: any, value2: any, tolerance: number): boolean {
   ...
   * @private
   */
  private findTopDifferences(
    allDifferences: OutcomeDifference[],
    maxCount: number
  ): OutcomeDifference[] {
    // Count occurrences of each difference pattern
    const diffCounts = new Map<string, { diff: OutcomeDifference; count: number }>();
   ...
   * Requirements: 3.2
   */
  async reconstructState(config: StateReconstructionConfig): Promise<ReconstructedState> {
    // Validate configuration
    ...
    try {
      ...
      let startState: ReconstructedState | null = null;
      ...
      throw new Error(`State reconstruction failed: ${(error as Error).message}`);
    }
  }
   ...
   * @private
   */
  private validateConfig(config: ReplayConfig): void {
    if (!config.timeRange) {
   ...
   * @private
   */
  private validateStateReconstructionConfig(config: StateReconstructionConfig): void {
   ...
   * @private
   */
  private createEmptyState(timestamp: number): ReconstructedState {
    return {
      timestamp,
   ...
   * @private
   */
  private cloneState(state: ReconstructedState): ReconstructedState {
    const clonedComponentStates = new Map<string, ComponentState>();
   ...
   * @private
   */
  private applyEventToState(state: ReconstructedState, event: DecisionEvent): void {
   ...
   * @private
   */
  private findClosestSnapshot(targetTimestamp: number): StateSnapshot | null {
   ...
   * @private
   */
  private async fetchEventsForStateReconstruction(
   ...
   * @private
   */
  private async fetchEventsInTimeRange(config: ReplayConfig): Promise<DecisionEvent[]> {
    const allEvents: DecisionEvent[] = [];
    let cursor: string | null = null;
    let hasMore = true;
   ...
   * @private
   */
  private async replayEvent(event: DecisionEvent, config: ReplayConfig): Promise<void> {
...
// Singleton instance
let replayEngineInstance: ReplayEngine | null = null;
 ...
 */
export function getReplayEngine(queryAPI?: DecisionQueryAPI): ReplayEngine {
