...
// Save manifest
fs.writeFileSync(path.join(__dirname, '../NeuralShell_Desktop/seal.manifest.json'), manifestJson);
console.log('[Boot Seal Generator] ✓ Saved seal.manifest.json');

// Save signature
fs.writeFileSync(path.join(__dirname, '../NeuralShell_Desktop/seal.manifest.sig'), signature);
console.log('[Boot Seal Generator] ✓ Saved seal.manifest.sig');

// Save private key (for development only - should be kept secure in production)
fs.writeFileSync(path.join(__dirname, '../NeuralShell_Desktop/seal.private.key'), privateKey);
console.log('[Boot Seal Generator] ⚠️  Saved seal.private.key (KEEP SECURE!)');

// Save private key (for development only - should be kept secure in production)
fs.writeFileSync('seal.private.key', privateKey);
console.log('[Boot Seal Generator] ⚠️  Saved seal.private.key (KEEP SECURE!)');
