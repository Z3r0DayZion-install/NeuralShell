...
test.describe('Recovery Center', () => {
  ...
  test.beforeAll(async () => {
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'neuralshell-e2e-recovery-'));
  ...
  test('opens recovery center and exports bundle', async () => {
    ...
    await page.getByTestId('mission-control-open-recovery-btn').click();

    await expect(page.getByTestId('recovery-center')).toBeVisible();
    await page.getByTestId('recovery-export-btn').click();
    await expect(page.getByText('Recovery bundle exported')).toBeVisible({ timeout: 10000 });
