...
test.describe('Split Workspace', () => {
  ...
  test.beforeAll(async () => {
    userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'neuralshell-e2e-splitworkspace-'));
  ...
  test('opens split workspace and routes right pane context', async () => {
    ...
    await expect(page.getByTestId('result-pane-proof')).toBeVisible();

    await page.getByTestId('snapshot-name-input').fill('Split Workspace Snapshot');
    await page.getByTestId('snapshot-capture-btn').click();
    await expect(page.locator('[data-testid^="snapshot-restore-"]').first()).toBeVisible({ timeout: 10000 });
  });
});