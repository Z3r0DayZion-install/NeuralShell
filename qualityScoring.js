 ...
 */
function calculateEffectivenessScore(event) {
 ...
 */
function calculateResponseTimeScore(durationMs, config) {
 ...
 */
function calculateCostImpactScore(event, config) {
 ...
 */
export function calculateQualityScoreWithBreakdown(event, config = {}) {
  const fullConfig = { ...DEFAULT_CONFIG, ...config };
 ...
 */
export function isLowQualityScore(score, threshold = 60) {
 ...
 */
export function explainQualityScore(breakdown) {
  const explanations = [];
  ...
  if (breakdown.effectiveness >= 80) {
    strengths.push('effectiveness');
  }
  if (breakdown.responseTime >= 80) {
    strengths.push('response time');
  }
  if (breakdown.costImpact >= 80) {
