...
contextBridge.exposeInMainWorld("neuralAPI", {
  readFile: (p, encoding) => ipcRenderer.invoke("read-file", p, encoding),
...
contextBridge.exposeInMainWorld("llmBridge", {
  ...
  modes: () => ipcRenderer.invoke("llm-modes"),
  autonomy: () => ipcRenderer.invoke("llm-autonomy"),
  apps: () => ipcRenderer.invoke("llm-apps"),
  optimizations: () => ipcRenderer.invoke("llm-optimizations"),
  exec: (command) => ipcRenderer.invoke("llm-exec", command),
  embed: (text) => ipcRenderer.invoke("llm-embed", text),
  vision: (payload) => ipcRenderer.invoke("llm-vision", payload),
  economy: () => ipcRenderer.invoke("llm-economy"),
  setQuineLock: (locked) => ipcRenderer.invoke("llm-set-quine-lock", locked),
  ...
  onCognitiveEvent: (callback) => {
    const handler = (_event, payload) => {
      if (typeof callback === "function") callback(payload);
    };
    ipcRenderer.on("cognitive:event", handler);
    return () => ipcRenderer.removeListener("cognitive:event", handler);
  }
});
...
contextBridge.exposeInMainWorld("telemetryBridge", {
  ...
  system: () => ipcRenderer.invoke("system-telemetry"),
  scanLegacy: () => ipcRenderer.invoke("scan-legacy-tears"),
  vaultGenome: () => ipcRenderer.invoke("vault-genome"),
  purge: () => ipcRenderer.invoke("system-purge")
});
...
contextBridge.exposeInMainWorld("authBridge", {
  ...
  onDecoy: (callback) => {
    ...
    ipcRenderer.on("auth:decoy", handler);
    return () => ipcRenderer.removeListener("auth:decoy", handler);
  }
});
